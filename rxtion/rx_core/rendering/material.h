...
rxcore_material_t *rxcore_material_create(const rxcore_material_prototype_t *prototype);
void rxcore_material_add_binding(rxcore_material_t *material, const char *uniform_name, void *data, uint32_t size);

#define rxcore_uniform_desc(uniform_stage, uniform_name, uniform_type) \
    { \
        .stage = uniform_stage, \
        .name = uniform_name, \
        .layout = { \
